...
public sealed class SensitiveDataProtector : ISensitiveDataProtector
{
    ...
    public SensitiveDataProtector(IDataProtectionProvider dataProtectionProvider, IConfiguration configuration)
    {
        ...
        if (_hashKey.Length < 32)
            throw new InvalidOperationException("Invalid Security:SensitiveDataHashKey configuration. The decoded key must be at least 32 bytes for HMAC-SHA256.");
    }

    /// <inheritdoc/>
    public string Protect(string normalizedValue) => _protector.Protect(normalizedValue);
